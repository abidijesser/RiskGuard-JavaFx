    ...
    public void rechercher(ActionEvent actionEvent) {
        ...
        if (!idStr.isEmpty()) {
            try {
                ...
                Assurance result = assuranceService.getAssuranceById(id);
                if (result != null) {
                    assuranceListView.setItems(FXCollections.observableArrayList(result));
                } else {
                    showAlert("Aucune assurance trouvée pour l'ID : " + id);
                }
            } catch (NumberFormatException e) {
                showAlert("Veuillez saisir un ID valide.");
            }
        } else {
            showAlert("Veuillez saisir un ID de recherche.");
        }
    }
